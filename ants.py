...
class Ant(Insect):
    ...
    def add_to(self, place):
      if place.ant is None:
            ...
            # BEGIN Problem 8
            #self is ant being added
            #place.ant is ant occupying first
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while place is not beehive:	
          if place.bees and distance <= self.max_range and distance >= self.min_range:	
...
class HungryAnt(Ant):
  ...
  def action(self, gamestate):  #If ant is not chewing eat else keep count of the chews
    if self.chewing > 0:
      self.chewing -= 1
