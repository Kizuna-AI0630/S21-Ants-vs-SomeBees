...
# python ants_gui.py -- run program
# python ok -q 01 --local -- test code change the number for next problem.
# python ok -q 03 --local -u -- unlock the code
...
class Insect:
  ...
  # set by Place.add_insect and Place.remove_insect
...
class FireAnt(Ant):
  ...
  def reduce_health(self, amount):
    ...
    # BEGIN Problem 5
    "*** YOUR CODE HERE ***"
    for b in list(self.place.bees): # Fire return same damage to all Bees
      b.reduce_health(amount)
    
    if self.health <= amount: # Fire dead, damage to all Bees before removed
...
class HungryAnt(Ant):
  ...
  def action(self, GameState):  #If ant is not chewing eat bee else keep count of the chews
    if self.chewing != 0:
      self.chewing -= 1
